-- Script pour restaurer/configurer l'activité ZERO TRACAS correctement
...
-- 1. Mettre à jour l'activité ZERO TRACAS existante avec la configuration correcte
UPDATE public.activities 
SET 
  name = 'ZERO TRACAS', -- Nom exact (en majuscules)
  category = 'exploration_bien_etre', -- Catégorie Exploration / Bien-être
  price_adult = 0, -- price_adult = 0 (les prix sont gérés par le système spécial ZERO TRACAS)
  ...
  notes = 'Service complet : Transfert + Visa + SIM (45€), Transfert + Visa (40€), Transfert 3 personnes (20€), Transfert +3 personnes (25€), Visa + SIM (40€), Visa seul (30€)', -- Notes descriptives
  transfers = '{}'::JSONB -- transfers vide (pas de transferts par quartier car déjà inclus dans les prix)
WHERE LOWER(name) LIKE '%zero tracas%' 
  AND LOWER(name) NOT LIKE '%hors zone%'
...
SELECT 
  ...
  'ZERO TRACAS', -- Nom exact (en majuscules)
  ...
  'Service complet : Transfert + Visa + SIM (45€), Transfert + Visa (40€), Transfert 3 personnes (20€), Transfert +3 personnes (25€), Visa + SIM (40€), Visa seul (30€)', -- Notes descriptives
