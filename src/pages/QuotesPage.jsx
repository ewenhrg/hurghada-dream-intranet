...
export function QuotesPage({ activities, quotes, setQuotes, user, draft, setDraft, onUsedDatesChange }) {
  ...
  // États pour les accordéons (client ouvert par défaut, autres fermés)
  ...
  // Fonction pour ouvrir automatiquement la section suivante
  const openNextSection = useCallback((currentSection) => {
    const sections = ['client', 'dates', 'globalAdults', 'activities'];
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-6 lg:p-8">
      ...
      {/* Layout 2 colonnes : Formulaire principal + Sidebar sticky */}
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 bg-white rounded-xl border border-slate-200 shadow-sm">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-indigo-500 to-blue-500 flex items-center justify-center shadow-sm">
            ...
            {/* Section Informations client - Accordéon */}
            <Accordion
              title="Informations client"
              icon="👤"
              defaultOpen={accordionStates.client}
              onToggle={(isOpen) => {
                setAccordionStates((prev) => ({ ...prev, client: isOpen }));
                if (isOpen && client.name && client.phone) {
                  openNextSection('client');
                }
              }}
              className="bg-[#f7f9fc]"
            >
            ...
            <Accordion
              ...
              icon="📅"
              defaultOpen={accordionStates.dates}
              onToggle={(isOpen) => {
                setAccordionStates((prev) => ({ ...prev, dates: isOpen }));
                if (isOpen && client.arrivalDate && client.departureDate) {
                  openNextSection('dates');
                }
              }}
              className="bg-[#f7f9fc]"
            >
            ...
            {/* Section Nombre d'adultes global - Accordéon */}
            <Accordion
              title="Nombre d'adultes global"
              icon="👥"
              defaultOpen={accordionStates.globalAdults}
              onToggle={(isOpen) => {
                setAccordionStates((prev) => ({ ...prev, globalAdults: isOpen }));
                if (isOpen && globalAdults) {
                  openNextSection('globalAdults');
                }
              }}
              className="bg-[#f7f9fc]"
            >
              <div className="space-y-4">
                ...
                <div className="max-w-xs">
                  <NumberInput
                    ...
                    placeholder="Ex: 2"
                    className="w-full text-base shadow-sm hover:shadow-md transition-all"
                  />
                  <div className="flex items-center gap-2 mt-3 px-3 py-2 bg-white/60 rounded-lg border border-slate-200">
            ...
            {/* Section Activités - Accordéon */}
            <Accordion
              title={`Activités (${computed.length} ${computed.length > 1 ? 'activités' : 'activité'})`}
              icon="🎯"
              defaultOpen={accordionStates.activities}
              onToggle={(isOpen) => {
                setAccordionStates((prev) => ({ ...prev, activities: isOpen }));
              }}
              className="bg-[#f7f9fc]"
            >
              <div className="space-y-5">
                {computed.map((c, idx) => (
                  <div 
                    key={idx} 
                    className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden animate-fade-in hover:scale-[1.01]" 
                    style={{ animationDelay: `${idx * 50}ms` }}
                  >
                    {/* En-tête de l'activité */}
                    <div className="bg-gradient-to-r from-blue-50/80 via-indigo-50/80 to-purple-50/80 border-b border-slate-200 px-5 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        <span className="inline-flex items-center justify-center w-10 h-10 rounded-lg bg-gradient-to-br from-blue-600 to-indigo-600 text-white text-sm font-bold shadow-sm">
                          {idx + 1}
                        </span>
                        <div>
                          <p className="text-sm font-bold text-slate-900">Activité #{idx + 1}</p>
                          {c.act && (
                            <p className="text-xs text-slate-600 mt-0.5 font-medium">{c.act.name}</p>
                          )}
                        </div>
                      </div>
                      <GhostBtn 
                    ...
                    <div className="p-5 md:p-6 space-y-5">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
                        <div>
                          <label className="block text-sm font-semibold text-slate-800 mb-2">Activité *</label>
                          <select
                            value={c.raw.activityId}
                            onChange={(e) => setItem(idx, { activityId: e.target.value })}
                            className="w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm hover:shadow-md"
                          >
              ...
              {c.act && c.act.name && c.act.name.toLowerCase().includes("speed boat") && (
                ...
                <GhostBtn 
                  ...
                  onClick={() => {
                    addItem();
                    setAccordionStates((prev) => ({ ...prev, activities: true }));
                  }} 
                  variant="primary" 
            ...
            <Accordion
              title="Notes"
              icon="📝"
              defaultOpen={accordionStates.notes}
              onToggle={(isOpen) => {
                setAccordionStates((prev) => ({ ...prev, notes: isOpen }));
              }}
              className="bg-[#f7f9fc]"
            >
            ...
            <PrimaryBtn 
              type="submit" 
              className="w-full text-base font-semibold py-4 shadow-lg hover:shadow-xl transition-all"
