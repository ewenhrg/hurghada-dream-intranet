...
export function PublicHotelsCataloguePage() {
  ...
  return (
    <div className="hd-public-catalog relative isolate flex min-h-screen flex-col overflow-x-hidden bg-catalog-bg font-catalog-sans text-catalog-body antialiased selection:bg-violet-200/55 selection:text-catalog-ink">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 top-0 z-0 h-[min(70vh,500px)] bg-[radial-gradient(ellipse_90%_70%_at_50%_-18%,rgba(167,139,250,0.22),transparent_58%)]"
      />
          ...
          {hotels.map((hotel, index) => (
            ...
            >
              ...
              <div className="flex grow flex-col p-5">
                <p className="line-clamp-3 text-sm leading-relaxed text-catalog-muted">
                  {hotel.description}
                </p>
                ...
                <div className="mt-5 grid gap-2">
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-violet-800 to-orange-600 px-4 py-3 text-sm font-bold text-white shadow-md shadow-violet-900/20 transition hover:brightness-110"
                  >
                    Ajouter au panier
                  </button>
                  <button
                    type="button"
                    onClick={() => openHotel(hotel.slug || hotel.id)}
                    className="inline-flex items-center justify-center gap-2 rounded-2xl border-2 border-violet-200 bg-white px-4 py-3 text-sm font-bold text-violet-800 transition hover:border-violet-400 hover:bg-violet-50"
                  >
                    Découvrir l’hôtel
